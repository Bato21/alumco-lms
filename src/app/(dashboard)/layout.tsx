...
export default async function DashboardLayout({
  ...
  const { data: profile, error: profileError } = await supabase
    ...
    .eq('id', user.id)
    .single() as { data: { full_name: string; role: UserRole; sede: string; area_trabajo: string[]; avatar_url: string | null } | null; error: unknown }
  ...
  return (
    <div className="min-h-screen bg-[#F8F9FA]">
      ...
      <WorkerSidebar
        ...
        area={profile.area_trabajo?.join(', ') ?? undefined}
