...
import { MapPin, Users, Loader2, Plus } from 'lucide-react'
...
export default function SedesClient({ sedes, workersPerSede }: SedesClientProps) {
  ...
  function handleToggle(sedeId: string, currentlyActive: boolean) {
    setTogglingId(sedeId)
    const toggle = async () => {
      const result = await toggleSedeAction(sedeId, !currentlyActive)
      if (result.error) setError(result.error)
      setTogglingId(null)
    }
    toggle()
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl shadow-[0_2px_8px_rgba(0,0,0,0.06)] p-6">
        ...
        <form action={handleCreate} className="flex gap-3">
          ...
          <button
            type="submit"
            disabled={isCreating}
            className="inline-flex items-center gap-2 px-5 h-11 rounded-xl bg-[#2B4FA0] text-white text-sm font-bold hover:bg-[#1e3c8a] transition-colors disabled:opacity-50"
          >
            {isCreating ? (
      ...
      {/* Lista de sedes */}
      {sedes.length === 0 ? (
        <div className="bg-white rounded-2xl shadow-[0_2px_8px_rgba(0,0,0,0.06)] p-16 text-center text-[#6B7280]">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {sedes.map(sede => {
            ...
            return (
              <div
                key={sede.id}
                className={`bg-white rounded-2xl shadow-[0_2px_8px_rgba(0,0,0,0.06)] p-6 flex flex-col gap-4 ${
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="font-bold text-[#1A1A2E] leading-tight">{sede.nombre}</p>
                      <p className="text-[10px] font-mono text-[#6B7280] mt-0.5">{sede.id}</p>
                    </div>
                  </div>
                ...
                <button
                  onClick={() => handleToggle(sede.id, sede.activa)}
                  disabled={isToggling}
                  className={`w-full h-10 flex items-center justify-center gap-2 rounded-xl border-2 font-semibold text-sm transition-colors disabled:opacity-50 ${
                    sede.activa
